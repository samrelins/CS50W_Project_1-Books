...
def search():
    if request.method == "POST":  
        ...
        books = db.execute("SELECT isbn, title, author, year FROM books "
                            + f"WHERE LOWER(isbn) LIKE LOWER('{search}') "
                            + f"OR LOWER(title) LIKE LOWER('{search}') "
                            + f"OR LOWER(author) LIKE LOWER('{search}') "
                            + "LIMIT 30").fetchall()
...
def book(isbn):
    ...
    if request.method == "POST":
        ...
        db.execute("insert into reviews (user_id, isbn, rating, review) " 
                    + f"values ('{session['user_id']}', '{isbn}', '{rating}', '{review}')")
        db.commit()
...
def edit_review(isbn):
    ...
    if request.method == "POST":
        ...
        if not review:
            ...
            return redirect(url_for("edit_review", isbn=isbn, error=error))
        if not rating:
            error = "Please submit a new rating"
            return redirect(url_for("edit_review", isbn=isbn, error=error))
        db.execute(f"UPDATE reviews SET review = '{review}', rating = '{rating}' "
                    + f"WHERE user_id = '{session['user_id']}' "
                    + f"AND isbn = '{isbn}'")
        db.commit()
...
def register():
    ...
    if request.method == "POST":
        ...
        if not username or not password or not confirmation_password or not email:
            ...
            return render_template("register.html", error=error)
        elif not db.execute(f"SELECT * FROM users WHERE username = '{username}'").rowcount == 0:
        ...
        db.execute("INSERT INTO users (username, password, email) "
                    + f"VALUES ('{username}', '{hashed_password}', '{email}')") 
        db.commit()
...
def login():
    ...
    if request.method == "POST":
        ...
        user = db.execute("SELECT user_id, password FROM users "
                            + f"WHERE username = '{username_or_email}' "
                            + f"OR email = '{username_or_email}'").fetchone()
