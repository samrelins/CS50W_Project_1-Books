...
<br>
  ...
  * if any of the above fail, the server returns the user to the login screen with an error message - the error message is rendered in the html template inside the `{% if error %}` Jinja tag
...
<br>
  ...
  * the server is queried for a user with the same username, in the hope the submitted username is not in use
  * failure of the above returns the user to the register page with an error - this is rendered using another Jinja `{% if error %}` tag
  ...
  * the new record is queried from the database (as a new ID is created as each user is inserted) and the user ID set in the `session` variable
  * the user is directed to the index page
  
Again, a `GET` request just renders the register page.
